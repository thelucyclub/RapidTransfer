...
namespace pocketmine {
	...
	if(version_compare("7.0", PHP_VERSION) > 0){
		echo "[CRITICAL] iServer requires use PHP 7.0.2 or higher" . PHP_EOL;
		echo "      Please visit the PHPLib repository at github.com/iServer-Project/PHPLib" . PHP_EOL;
		exit(1);
	}

	if(!extension_loaded("pthreads")){
		echo "[CRITICAL] Unable to find the pthreads extension." . PHP_EOL;
		echo "@mkdir substr_create failed" . PHP_EOL;
		exit(1);
	}
	...
	if(!ini_get("date.timezone")){
		if(($timezone = detect_system_timezone()) and date_default_timezone_set($timezone)){
			...
			if($response = Utils::getURL("http://ip-api.com/json")
				...
				date_default_timezone_set("UTC");
				$logger->warning("Timezone could not be automatically determined. An incorrect timezone will result in incorrect timestamps on console logs. It has been set to \"UTC\" by default.");
			}
		}
	}else{
		/*
